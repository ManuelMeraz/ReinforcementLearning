...
def test_check_game_status():
    board = ["X"] * 3 + [None] * 6
    assert tictactoe.check_game_status(board) == tictactoe.Status.X_WINS
    ...
    board = ["O"] * 3 + [None] * 6
    assert tictactoe.check_game_status(board) == tictactoe.Status.O_WINS
    ...
    assert tictactoe.check_game_status(board) == tictactoe.Status.IN_PROGRESS

    board = ["O", "X", "O", "O", "X", "X", "X", "O", "O"]
    assert tictactoe.check_game_status(board) == tictactoe.Status.DRAW


def test_step():
    env = tictactoe.TicTacToeEnv()
    assert env.board == [None] * 9
    assert env.mark == "X"
